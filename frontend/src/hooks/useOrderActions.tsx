...
import {WhatsAppTicketMessageModal} from "../components/modals/WhatsAppTicketMessageModal";
...
export const useOrderActions = ({eventId, onManage, onEdit}: UseOrderActionsOptions) => {
    ...
    const [isWhatsAppModalOpen, whatsAppModal] = useDisclosure(false);
    ...
    const getOrderActions = (order: Order): EntityAction[] => {
        ...
        const actions: (EntityAction | false)[] = [
            ...
            order.status === 'COMPLETED' && (order.attendees ?? []).some(attendee => attendee.status === 'ACTIVE') && {
                key: 'whatsapp',
                label: t`Send tickets via WhatsApp`,
                icon: <IconBrandWhatsapp size={14}/>,
                onClick: () => openModal(order, whatsAppModal),
                group: 'primary',
            },
    ...
    const orderActionModals = selectedOrderId && (
        <>
            ...
            {isWhatsAppModalOpen && <WhatsAppTicketMessageModal
    ...
    return {
        ...
        openWhatsAppModal: (order: Order) => openModal(order, whatsAppModal),
